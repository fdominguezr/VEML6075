/* VEML6075_t3 Basic Example Code
 by: Kris Winer
 date: June 6, 2016
 Edition by: Francisco Dominguez Roman
 ...
 getting properly scaled UVA/B intensity data out. Sketch runs on the Arduino Mega2560 R3.
...
the  best  market-proven  reliability.
 ...
 VDD ---------------------- 3.3V or any digital pin i.e., digitalWrite(HIGH) IF the board is powered by 3.3V. NOT VALID for Arduino Mega
 SDA -----------------------pin 20 (SDA)
 SCL -----------------------pin 21 (SCL)
 GND ---------------------- GND or any digital pin i.e., digitalWrite(LOW) IF the board is powered by 3.3V. NOT VALID for Arduino Mega
 
*/
  
// #include <i2c_t3.h> // Only for Tensi board
#include <Wire.h>
...
//  Wire.begin(I2C_MASTER, 0x00, I2C_PINS_16_17, I2C_PULLUP_EXT, I2C_RATE_400);
  ...
  // if(deviceID == 0x26) // Modified so it actually 
  if(1) // it passes always due to my board ID is very strange
  {
...
{  
  ...
  // Trying to workout a total value for UV intensity = UVA intensity + UVB intensity
  Serial.print("TOTAL UV intensity = "); Serial.print((((float)UVData[2])/UVBSensitivity)+(((float)UVData[0])/UVASensitivity), 2); Serial.println(" microWatts/cm^2");
  ...
  delay(ITime+20); // Original delay
  Serial.flush(); // Trying to make serial readings more clear
}
